...
- [x] Исправить IDOR friend request response.
  - [x] Валидировать `requestId` как строго положительное целое число и принимать только `accept`/`reject`.
  - [x] Выполнять ответ одной атомарной операцией только по `id + to_user_id + status='pending'`.
  - [x] Возвращать одинаковый `404` для чужой, отсутствующей и уже обработанной заявки, не раскрывая её существование.
  - [x] Проверить на реальной БД сценарий с владельцем и посторонним пользователем: чужой ответ — `404` без изменения, ответ владельца — `200`, повтор — `404`; временные данные удалены (14.09.2026).
- [x] Закрыть stored-XSS класс в upload pipeline.
  - [x] Принимать upload только в память и не сохранять клиентское имя или расширение.
  - [x] Fail-closed декодировать только JPEG/PNG/GIF/WebP и перекодировать содержимое в новый WebP через Sharp; ограничить размер, число кадров, пиксели и итоговые габариты.
